...
$existingCats = $ToryHub->get_list_safe("SELECT DISTINCT category FROM `expenses` WHERE `warehouse` = 'vn' ORDER BY category ASC", []);
...
$_mRow = $ToryHub->get_row_safe("SELECT COALESCE(SUM(amount),0) as total FROM `expenses` WHERE `expense_date` BETWEEN ? AND ? AND `warehouse` = 'vn'", [$monthStart, $monthEnd]);
$totalMonth = floatval($_mRow['total']);
$countMonth = $ToryHub->num_rows_safe("SELECT id FROM `expenses` WHERE `expense_date` BETWEEN ? AND ? AND `warehouse` = 'vn'", [$monthStart, $monthEnd]) ?: 0;
...
$_pRow = $ToryHub->get_row_safe("SELECT COALESCE(SUM(amount),0) as total FROM `expenses` WHERE `expense_date` BETWEEN ? AND ? AND `warehouse` = 'vn'", [$prevMonthStart, $prevMonthEnd]);
...
$catSums = $ToryHub->get_list_safe("SELECT category, COALESCE(SUM(amount),0) as total, COUNT(*) as cnt FROM `expenses` WHERE `expense_date` BETWEEN ? AND ? AND `warehouse` = 'vn' GROUP BY category ORDER BY total DESC", [$monthStart, $monthEnd]);

// Năm có dữ liệu
$yearsData = $ToryHub->get_list_safe("SELECT DISTINCT YEAR(expense_date) as y FROM `expenses` WHERE `warehouse` = 'vn' ORDER BY y DESC", []);
...
?>
            ...
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    ...
                    <div class="d-flex gap-2">
                        <a href="<?= base_url('ajaxs/staffvn/expenses-export.php') ?>?month=<?= $filterMonth ?>&year=<?= $filterYear ?>&category=<?= urlencode($filterCat) ?>" class="btn btn-success">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-end justify-content-between mt-2">
                            ...
                            <div class="avatar-sm flex-shrink-0">
                                <span class="avatar-title bg-danger-subtle rounded fs-3"><i class="ri-money-dollar-circle-line text-danger"></i></span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form method="GET" action="<?= base_url('staffvn/expenses') ?>">
                            <div class="row g-3 align-items-end">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <a href="<?= base_url('staffvn/expenses') ?>" class="btn btn-outline-secondary"><?= __('Reset') ?></a>
                                </div>
                            </div>
                        </form>
                    </div>
                ...
                <div class="modal-content">
                    ...
                    <form id="form-add-expense">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label class="form-label"><?= __('Số tiền') ?> (VNĐ) <span class="text-danger">*</span></label>
                                <input type="number" class="form-control" name="amount" min="0" step="1" required>
                            </div>
                            <div class="mb-3">
                ...
                <div class="modal-content">
                    ...
                    <form id="form-edit-expense">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label class="form-label"><?= __('Số tiền') ?> (VNĐ) <span class="text-danger">*</span></label>
                                <input type="number" class="form-control" name="amount" id="edit-amount" min="0" step="1" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label"><?= __('Ngày chi') ?> <span class="text-danger">*</span></label>
...
$(function() {
    ...
    $('#form-add-expense').on('submit', function(e) {
        ...
        $.ajax({
            url: '<?= base_url('ajaxs/staffvn/expenses.php') ?>',
            type: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
    ...
    // Edit expense - open modal (VND directly)
