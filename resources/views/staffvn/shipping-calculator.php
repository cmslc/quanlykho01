...
if ($filterType !== 'wholesale' && $filterCargo !== 'difficult') {
    ...
    if ($filterDateFrom) {
        $bagWhere .= " AND DATE(b.create_date) >= ?";
        $bagParams[] = $filterDateFrom;
    }
    if ($filterDateTo) {
        $bagWhere .= " AND DATE(b.create_date) <= ?";
        $bagParams[] = $filterDateTo;
    }

    $sealedBags = $ToryHub->get_list_safe(
        "SELECT b.id as bag_id, b.bag_code, b.status as bag_status, b.images as bag_images,
...
if ($filterType !== 'bag') {
    ...
    if ($filterDateFrom) {
        $orderWhere .= " AND DATE(o.create_date) >= ?";
    ...
    $wholesaleOrders = $ToryHub->get_list_safe(
        "SELECT o.id, o.product_code, o.cargo_type, o.product_image, o.customer_id,
            ...
            o.weight_charged as order_weight_charged, o.volume_actual,
...
if (!empty($wholesaleOrders) && ($filterWeightMin !== null || $filterWeightMax !== null || $filterCbmMin !== null || $filterCbmMax !== null)) {
    $wholesaleOrders = array_values(array_filter($wholesaleOrders, function($o) use ($filterWeightMin, $filterWeightMax, $filterCbmMin, $filterCbmMax) {
        $wC = floatval($o['order_weight_charged'] ?? 0); $wA = floatval($o['order_weight_actual'] ?? 0); $pkgWC = floatval($o['pkg_weight_charged'] ?? 0); $pkgWA = floatval($o['pkg_weight_actual'] ?? 0); $w = $pkgWA > 0 ? $pkgWA : ($wA > 0 ? $wA : ($pkgWC > 0 ? $pkgWC : $wC));
        $c = $o['total_cbm'] ?? 0;
        if (floatval($o['volume_actual'] ?? 0) > 0) $c = floatval($o['volume_actual']);
        if ($filterWeightMin !== null && $w < $filterWeightMin) return false;
        if ($filterWeightMax !== null && $w > $filterWeightMax) return false;
        if ($filterCbmMin !== null && $c < $filterCbmMin) return false;
        if ($filterCbmMax !== null && $c > $filterCbmMax) return false;
...
foreach ($wholesaleOrders as $order) {
    $wC = floatval($order['order_weight_charged'] ?? 0); $wA = floatval($order['order_weight_actual'] ?? 0); $pkgWC = floatval($order['pkg_weight_charged'] ?? 0); $pkgWA = floatval($order['pkg_weight_actual'] ?? 0); $w = $pkgWA > 0 ? $pkgWA : ($wA > 0 ? $wA : ($pkgWC > 0 ? $pkgWC : $wC));
    $c = $order['total_cbm'] ?? 0; if (floatval($order['volume_actual'] ?? 0) > 0) $c = floatval($order['volume_actual']); $cargo = $order['cargo_type'] ?? 'easy';
    ...
    $allRows[] = ['type' => 'order', 'data' => $order, 'weight' => floatval($w), 'cbm' => floatval($c), 'cost' => $cost, 'pkg_count' => intval($order['pkg_count'] ?? 0), 'cargo' => $cargo, 'rate_kg' => $rkg, 'rate_cbm' => $rcbm, 'domestic_cost' => floatval($order['domestic_cost'] ?? 0), 'ex_rate' => floatval($order['order_exchange_rate'] ?? 0)];
}
...
// Grand totals
$grandPkgs = 0; $grandWeight = 0; $grandCbm = 0; $grandTotalCost = 0;
$warnRows = 0;
$_exRate = get_exchange_rate();
foreach ($allRows as &$row) {
    $grandPkgs += $row['pkg_count'];
    $grandWeight += $row['weight'];
    $grandCbm += $row['cbm'];
    $_rowRate = ($row['ex_rate'] ?? 0) > 0 ? $row['ex_rate'] : $_exRate;
    $grandTotalCost += $row['cost']['max'] + $row['domestic_cost'] * $_rowRate;
...
?>
                ...
                <div class="card card-animate">
                    <div class="card-body">
                        <div class="d-flex align-items-end justify-content-between mt-2">
                            <div>
                                <p class="text-uppercase fw-medium text-muted mb-0"><?= __('Tổng kiện') ?></p>
                                <h4 class="fs-22 fw-semibold mt-4 mb-0"><?= number_format($grandPkgs) ?></h4>
                            </div>
                            <div class="avatar-sm flex-shrink-0">
                                <span class="avatar-title bg-info-subtle rounded fs-3"><i class="ri-inbox-line text-info"></i></span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="card card-animate">
                    <div class="card-body">
                        <div class="d-flex align-items-end justify-content-between mt-2">
                            <div>
                                <p class="text-uppercase fw-medium text-muted mb-0"><?= __('Cân nặng') ?></p>
                                <h4 class="fs-22 fw-semibold mt-4 mb-0 text-primary"><?= fnum($grandWeight, 1) ?> <small class="fs-14">kg</small></h4>
                            </div>
                            <div class="avatar-sm flex-shrink-0">
                                <span class="avatar-title bg-primary-subtle rounded fs-3"><i class="ri-scales-line text-primary"></i></span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-xl-3 col-md-6">
                <div class="card card-animate">
                    <div class="card-body">
                        <div class="d-flex align-items-end justify-content-between mt-2">
                            <div>
                                <p class="text-uppercase fw-medium text-muted mb-0"><?= __('Số khối') ?></p>
                                <h4 class="fs-22 fw-semibold mt-4 mb-0 text-primary"><?= fnum($grandCbm, 2) ?> <small class="fs-14">m&sup3;</small></h4>
                            </div>
                            <div class="avatar-sm flex-shrink-0">
                                <span class="avatar-title bg-warning-subtle rounded fs-3"><i class="ri-box-3-line text-warning"></i></span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-end justify-content-between mt-2">
                            <div>
                                ...
                                <h4 class="fs-22 fw-semibold mt-4 mb-0 text-danger"><?= format_vnd($grandTotalCost) ?></h4>
                            </div>
                            <div class="avatar-sm flex-shrink-0">
        ...
        <!-- Filter Bar -->
        <?php
        $baseUrl = base_url('staffvn/shipping-calculator');
        ...
        $currentFilters = [
            ...
            'sort' => $filterSort, 'dir' => $filterSortDir, 'per_page' => $perPage
        ];
        ?>
        <div class="row mb-3">
            <div class="col-12">
                    ...
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="type" class="form-select">
                                ...
                                <option value="bag" <?= $filterType === 'bag' ? 'selected' : '' ?>><?= __('Mã bao') ?></option>
                            </select>
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            <label class="form-label mb-1 fs-12"><?= __('Thời gian') ?></label>
            ...
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body">
                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($pagedRows as $row):
                                        ...
                                        $_rRate = ($row['ex_rate'] ?? 0) > 0 ? $row['ex_rate'] : $_exRate;
                                        $warnClass = '';
                                    ?>
                                    <?php if ($row['type'] === 'bag'):
                                        $bag = $row['data'];
                                    ...
                                    <tr class="<?= $warnClass ?>">
                                        ...
                                        <td class="align-middle">
                                            <a href="<?= base_url('staffvn/bags-packing?id=' . $bag['bag_id']) ?>"><strong><?= htmlspecialchars($bag['bag_code']) ?></strong></a>
                                            <?php $bsl = $bagStatusLabels[$bag['bag_status']] ?? $bagStatusLabels['sealed']; ?>
                                            <div class="mt-1"><span class="badge bg-<?= $bsl['bg'] ?> text-<?= $bsl['text'] ?> fs-12 px-2 py-1"><i class="<?= $bsl['icon'] ?? 'ri-time-line' ?> align-middle me-1"></i><?= __($bsl['label']) ?></span></div>
                                            <?php if ($pkgCount > 0): ?>
                                        ...
                                        <td class="align-middle">
                                            ...
                                            <div class="domestic-vnd-label text-muted" style="font-size:11px;"><?= $row['domestic_cost'] > 0 ? '≈ ' . format_vnd($row['domestic_cost'] * $_rRate) : '' ?></div>
                                            <div class="text-muted" style="font-size:11px;"><i class="ri-exchange-line me-1"></i><?= __('Tỉ giá') ?>: <?= number_format($_rRate, 0, '', '.') ?></div>
                                        </td>
                                        <td class="align-middle total-cost-cell"><strong class="text-danger"><?= format_vnd($shipCost['max'] + $row['domestic_cost'] * $_rRate) ?></strong></td>
                                    </tr>
                                    <?php else:
                                        $order = $row['data'];
                                    ...
                                    <tr class="<?= $warnClass ?>">
                                        ...
                                        <td class="align-middle">
                                            ...
                                            <a href="<?= base_url('staffvn/orders-detail?id=' . $order['id']) ?>"><strong><?= htmlspecialchars($order['product_code']) ?></strong></a>
                                            <?php else: ?>
                                            <a href="<?= base_url('staffvn/orders-detail?id=' . $order['id']) ?>" class="text-muted">#<?= $order['id'] ?></a>
                                            <?php endif; ?>
                        ...
                        <?php if ($totalPages > 1 || $totalRows > 10): ?>
            ...
            <div class="col-12">
                <div class="card card-animate">
                    <div class="card-header">
                        <h6 class="card-title mb-0"><i class="ri-information-line me-1"></i><?= __('Ghi chú') ?></h6>
                    </div>
                    <div class="card-body pt-2">
                        <ul class="list-unstyled mb-0 text-muted" style="font-size:13px; line-height:2;">
                            <li><i class="ri-calculator-line me-1 text-primary"></i><strong><?= __('Cước vận chuyển') ?></strong> = MAX(Kg × <?= __('Đơn giá') ?>/kg, M³ × <?= __('Đơn giá') ?>/m³) — <span class="fw-bold text-danger"><?= __('giá trị lớn hơn') ?></span> <?= __('được áp dụng') ?></li>
                            <li><i class="ri-money-cny-circle-line me-1 text-primary"></i><strong><?= __('Tổng cước') ?></strong> = <?= __('Cước vận chuyển') ?> + <?= __('Cước nội địa') ?> (¥) × <?= __('Tỉ giá') ?></li>
                            <li><i class="ri-price-tag-3-line me-1 text-primary"></i><strong><?= __('Đơn giá mặc định') ?>:</strong> <?= __('Hàng dễ') ?> = <?= number_format($shippingRates['road']['easy']['per_kg'], 0, '', '.') ?>đ/kg | <?= number_format($shippingRates['road']['easy']['per_cbm'], 0, '', '.') ?>đ/m³ · <?= __('Hàng khó') ?> = <?= number_format($shippingRates['road']['difficult']['per_kg'], 0, '', '.') ?>đ/kg | <?= number_format($shippingRates['road']['difficult']['per_cbm'], 0, '', '.') ?>đ/m³</li>
                            <li><i class="ri-exchange-cny-line me-1 text-primary"></i><strong><?= __('Tỉ giá hiện tại') ?>:</strong> 1¥ = <?= number_format($_exRate, 0, '', '.') ?>đ</li>
                            <li><i class="ri-edit-line me-1 text-primary"></i><?= __('Đơn giá và cước nội địa có thể chỉnh sửa trực tiếp trên bảng') ?></li>
                            <li><i class="ri-shopping-bag-line me-1 text-primary"></i><strong><?= __('Mã hàng') ?>:</strong> <?= __('Hiển thị tất cả mã hàng trong Hàng Lô') ?></li>
                            <li><i class="ri-archive-line me-1 text-primary"></i><strong><?= __('Mã bao') ?>:</strong> <?= __('Hiển thị bao hàng có trạng thái') ?>: <?php foreach (['sealed','loading','shipping','arrived'] as $_s): $__l = $bagStatusLabels[$_s]; ?><span class="badge bg-<?= $__l['bg'] ?> text-<?= $__l['text'] ?> fs-11 me-1"><i class="<?= $__l['icon'] ?> me-1"></i><?= __($__l['label']) ?></span><?php endforeach; ?></li>
                        </ul>
                    </div>
                </div>
            </div>
...
$(function(){
    ...
    function renderFilterCustomerDropdown(q) {
        ...
        results.slice(0, 50).forEach(function(c){
            html += '<div class="filter-cust-option px-3 py-2 border-bottom" data-id="' + c.id + '" data-label="' + $('<span>').text(c.label).html() + '" style="cursor:pointer;">'
                + '<span>' + $('<span>').text(c.label).html() + '</span></div>';
        });
        $dd.html(html).show();
    }
    ...
    var pkgAjaxUrl = '<?= base_url('ajaxs/staffvn/packages.php') ?>';
    var calcAjaxUrl = '<?= base_url('ajaxs/staffvn/shipping-calculator.php') ?>';
    var expandedOrders = {};
    ...
    $(document).on('click', '.btn-expand-pkgs', function(e){
        ...
        $.post(pkgAjaxUrl, { request_name: 'get_order_packages', order_id: orderId, csrf_token: csrfToken }, function(res){
            if (res.status === 'success') {
                var statusLabels = { 'cn_warehouse': '<?= __('Tại kho TQ') ?>', 'packed': '<?= __('Đã đóng bao') ?>', 'loading': '<?= __('Đang xếp xe') ?>', 'shipping': '<?= __('Đang vận chuyển') ?>', 'vn_warehouse': '<?= __('Đã về kho Việt Nam') ?>', 'delivered': '<?= __('Đã giao') ?>', 'returned': '<?= __('Đã trả') ?>', 'damaged': '<?= __('Hư hỏng') ?>' };
                ...
                html += '<th><?= __('Mã kiện') ?></th><th><?= __('Cân nặng / Số khối') ?></th><th><?= __('Kích thước') ?></th><th><?= __('Cước') ?></th><th><?= __('Trạng thái') ?></th>';
                html += '</tr></thead><tbody>';
                ...
                groups.forEach(function(group){
                    ...
                    if (pkgs.length === 1) {
                        var pkgCode = first.package_code || ('#' + first.id);
                        ...
                        html += '<td><strong>' + pkgCode + '</strong></td>';
                        ...
                        var firstCode = pkgs[0].package_code || ('#' + pkgs[0].id);
                        var lastCode = pkgs[pkgs.length - 1].package_code || ('#' + pkgs[pkgs.length - 1].id);
                        ...
                        html += '<td><a href="#" class="btn-expand-group text-decoration-none" data-group-id="' + groupId + '"><strong>' + firstCode + ' ~ ' + lastCode + '</strong> <span class="badge bg-primary-subtle text-primary">' + pkgs.length + ' <?= __('kiện') ?></span> <i class="ri-arrow-down-s-line grp-icon"></i></a></td>';
    ...
    $('#btn-export-excel').on('click', function(){
        var headers = ['<?= __('Mã hàng / Mã bao') ?>', '<?= __('Loại') ?>', '<?= __('Phân loại') ?>', '<?= __('Khách hàng') ?>', '<?= __('Cân nặng (kg)') ?>', '<?= __('Số khối (m³)') ?>', '<?= __('Đơn giá / kg') ?>', '<?= __('Đơn giá / m³') ?>', '<?= __('Cước theo kg (₫)') ?>', '<?= __('Cước theo m³ (₫)') ?>', '<?= __('Cước nội địa (¥)') ?>', '<?= __('Cước nội địa (₫)') ?>', '<?= __('Tổng cước (₫)') ?>'];
        var rows = [headers];
        ...
        ws['!cols'] = headers.map(function(h, i){ return {wch: Math.max(h.length + 2, 12)}; });
        var wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
        XLSX.writeFile(wb, '<?= __('tich-cuoc-don-hang') ?>_' + new Date().toISOString().slice(0,10) + '.xlsx');
    });
