...
$(function(){
    ...
    $(document).on('click', '.btn-expand-pkgs', function(e){
        ...
        $.post(pkgAjaxUrl, { request_name: 'get_order_packages', order_id: orderId, csrf_token: csrfToken }, function(res){
            if (res.status === 'success') {
                var statusLabels = { 'cn_warehouse': '<?= __('Tại kho TQ') ?>', 'packed': '<?= __('Đã đóng bao') ?>', 'loading': '<?= __('Xếp xe') ?>', 'shipping': '<?= __('Vận chuyển') ?>', 'vn_warehouse': '<?= __('Kho VN') ?>', 'delivered': '<?= __('Đã giao') ?>', 'returned': '<?= __('Đã trả') ?>', 'damaged': '<?= __('Hư hỏng') ?>' };
