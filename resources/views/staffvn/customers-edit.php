...
if (!$customer) {
    redirect(base_url('staffvn/customers-list'));
}
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form id="form-edit-customer">
                            ...
                            <a href="<?= base_url('staffvn/customers-list') ?>" class="btn btn-secondary"><?= __('Quay lại') ?></a>
                        </form>
                    </div>
                </div>
            </div>
...
$('#form-edit-customer').on('submit', function(e){
    e.preventDefault();
    $.post('<?= base_url('ajaxs/staffvn/customers.php') ?>', $(this).serialize(), function(res){
        if(res.status == 'success'){
            Swal.fire({icon: 'success', title: res.msg, timer: 1500, showConfirmButton: false}).then(function(){
                window.location.href = '<?= base_url('staffvn/customers-list') ?>';
            });
        } else {
