...
$_mRow = $ToryHub->get_row_safe("SELECT COALESCE(SUM(amount),0) as total, COALESCE(SUM(amount / COALESCE(NULLIF(exchange_rate,0), ?)),0) as total_cny FROM `expenses` WHERE `expense_date` BETWEEN ? AND ?", [$exchangeRate, $monthStart, $monthEnd]);
$totalMonth = floatval($_mRow['total']);
$totalMonthCny = floatval($_mRow['total_cny']);
$countMonth = $ToryHub->num_rows_safe("SELECT id FROM `expenses` WHERE `expense_date` BETWEEN ? AND ?", [$monthStart, $monthEnd]) ?: 0;
...
$_pRow = $ToryHub->get_row_safe("SELECT COALESCE(SUM(amount),0) as total, COALESCE(SUM(amount / COALESCE(NULLIF(exchange_rate,0), ?)),0) as total_cny FROM `expenses` WHERE `expense_date` BETWEEN ? AND ?", [$exchangeRate, $prevMonthStart, $prevMonthEnd]);
                        ...
                        <form method="GET" action="<?= base_url('admin/expenses') ?>">
                            <div class="row g-3 align-items-end">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <a href="<?= base_url('admin/expenses') ?>" class="btn btn-outline-secondary"><?= __('Reset') ?></a>
                                </div>
                            </div>
                        </form>
                    </div>
