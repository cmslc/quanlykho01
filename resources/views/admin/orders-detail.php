...
$order = $ToryHub->get_row_safe("SELECT o.*, c.fullname as customer_name, c.phone as customer_phone
...
// Get packages with shipment info
$Packages = new Packages();
$order_packages = $ToryHub->get_list_safe(
    "SELECT p.*, s.shipment_code, s.truck_plate
     FROM `packages` p
     INNER JOIN `package_orders` po ON p.id = po.package_id
     LEFT JOIN `shipment_packages` sp ON p.id = sp.package_id
     LEFT JOIN `shipments` s ON sp.shipment_id = s.id
     WHERE po.order_id = ? ORDER BY p.create_date ASC", [$id]
);
...
?>
        ...
        $totalPkgWeight = 0; $totalPkgCbm = 0;
        foreach ($order_packages as $_p) {
            $totalPkgWeight += floatval($_p['weight_actual'] ?? 0);
            $totalPkgCbm += floatval($_p['length_cm'] ?? 0) * floatval($_p['width_cm'] ?? 0) * floatval($_p['height_cm'] ?? 0) / 1000000;
        }
        // Ưu tiên volume_actual nếu đã nhập tay
        if (floatval($order['volume_actual'] ?? 0) > 0) {
            $totalPkgCbm = floatval($order['volume_actual']);
        }
                            ...
                            <div class="table-responsive">
                                <table id="tbl-packages" class="table table-bordered table-sm mb-0">
                                    ...
                                    <tbody id="tbody-grouped" class="d-none">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                $grpShipments = [];
                                                foreach ($gpkgs as $gp) {
                                                    if (!empty($gp['shipment_code']) && in_array($gp['status'], ['loading', 'shipping', 'vn_warehouse', 'delivered'])) {
                                                        $grpShipments[$gp['shipment_code']] = $gp['truck_plate'] ?? '';
                                                    }
                                                }
                                                if (!empty($grpShipments)):
                                                    foreach ($grpShipments as $sc => $tp): ?>
                                                <div>
                                                    <span class="badge bg-dark-subtle text-dark"><?= htmlspecialchars($sc) ?></span>
                                                    <?php if ($tp): ?><br><small class="text-muted"><i class="ri-truck-line"></i> <?= htmlspecialchars($tp) ?></small><?php endif; ?>
                                                </div>
                                                <?php endforeach; else: ?>
                                    ...
                                    <tbody id="tbody-ungrouped">
                                        ...
                                        ?>
                                        <tr>
                                            <td><a href="<?= base_url('admin/packages-detail?id='.$pkg['id']) ?>"><strong><?= htmlspecialchars($pkg['package_code']) ?></strong></a></td>
                                            <?php if ($isRetail): ?>
                                            ...
                                            <td>
                                                <?php if (!empty($pkg['shipment_code']) && in_array($pkg['status'], ['loading', 'shipping', 'vn_warehouse', 'delivered'])): ?>
                                                <span class="badge bg-dark-subtle text-dark"><?= htmlspecialchars($pkg['shipment_code']) ?></span>
                                                <?php if ($pkg['truck_plate']): ?><br><small class="text-muted"><i class="ri-truck-line"></i> <?= htmlspecialchars($pkg['truck_plate']) ?></small><?php endif; ?>
                                                <?php else: ?>
                                                <span class="text-muted">-</span>
                                                <?php endif; ?>
                                            </td>
                                    ...
                                    <tfoot class="table-light">
                                        <tr class="fw-bold">
                                            <td><?= __('Tổng') ?> (<?= $totalPkgs ?> <?= __('kiện') ?>)</td>
                                            <?php if ($isRetail): ?><td></td><?php endif; ?>
                                            <td><?= $totalPkgWeight > 0 ? fnum($totalPkgWeight, 2) . ' kg' : '<span class="text-muted fw-normal">N/A</span>' ?></td>
                                            <td></td>
                                            <td><?= $totalPkgCbm > 0 ? number_format($totalPkgCbm, 4, '.', '') . ' m³' : '<span class="text-muted fw-normal">N/A</span>' ?></td>
                                            <td></td>
                                            <td></td>
                                        </tr>
                                    </tfoot>
                        ...
                        <div class="row">
                                ...
                                <table class="table table-borderless mb-0">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="<?= base_url('admin/customers-detail?id=' . $order['customer_id']) ?>"><?= htmlspecialchars($order['customer_name']) ?></a>
                            ...
                            <div class="col-md-6">
                                <table class="table table-borderless mb-0">
                                    ...
                                    <?php if ($totalPkgWeight > 0): ?>
                                    <tr>
                                        <td class="text-muted"><?= __('Tổng cân nặng mã hàng') ?></td>
                                        <td class="fw-bold"><?= fnum($totalPkgWeight, 2) ?> kg</td>
                                    </tr>
                                    <?php endif; ?>
                                    <?php if ($totalPkgCbm > 0): ?>
                                    <tr>
                                        <td class="text-muted"><?= __('Số khối') ?></td>
                                        <td class="fw-bold"><?= number_format($totalPkgCbm, 4, '.', '') ?> m³</td>
                                    </tr>
                                    <?php endif; ?>
