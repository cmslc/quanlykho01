...
$totalOrders = $ToryHub->get_row_safe("SELECT COUNT(*) as cnt FROM `orders` WHERE DATE(create_date) >= ? AND DATE(create_date) <= ?", [$filterDateFrom, $filterDateTo])['cnt'] ?: 0;
...
$cancelledOrders = $ToryHub->get_row_safe("SELECT COUNT(*) as cnt FROM `orders` WHERE `status` = 'cancelled' AND DATE(create_date) >= ? AND DATE(create_date) <= ?", [$filterDateFrom, $filterDateTo])['cnt'] ?: 0;
                    ...
                    <div class="card-body">
                        <form method="GET" action="<?= base_url('admin/reports-orders') ?>" class="row g-3 align-items-end">
                            ...
                            <div class="col-md-3">
                                ...
                                <a href="<?= base_url('admin/reports-orders') ?>" class="btn btn-outline-secondary"><?= __('Reset') ?></a>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="card card-animate">
                    <div class="card-body">
                        <div class="d-flex align-items-end justify-content-between mt-2">
                            ...
                            <div class="avatar-sm flex-shrink-0">
                                <span class="avatar-title bg-info-subtle rounded fs-3"><i class="ri-bar-chart-line text-info"></i></span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title mb-0"><?= __('Biểu đồ đơn hàng') ?></h5>
                    </div>
                    <div class="card-body">
                        <div id="orders-chart" style="min-height: 300px;"></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
(function(){
    var chartData = <?= json_encode(array_reverse($dailyOrders)) ?>;
    var categories = chartData.map(function(d){ return d.day; });
    var counts = chartData.map(function(d){ return parseInt(d.cnt); });
    var totals = chartData.map(function(d){ return parseFloat(d.total); });
    ...
    new ApexCharts(document.querySelector('#orders-chart'), {
        ...
        tooltip: {
            shared: true,
            y: { formatter: function(val, opts) { return opts.seriesIndex === 1 ? formatVND(val) : val + ' <?= __("đơn") ?>'; } }
        },
