                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    <?php if (empty($bags)): ?>
                                    <tr><td colspan="10" class="text-center text-muted py-4"><?= __('Chưa có bao hàng nào') ?></td></tr>
                                    <?php endif; ?>
                                    ...
                                    <tr class="bag-row" data-bag-id="<?= $bag['id'] ?>" style="cursor:pointer;">
                                        <td class="align-middle text-muted"><?= $rowNum ?></td>
                                        ...
                                        <td class="align-middle"><?= number_format($bag['total_weight'], 2) ?> kg</td>
                                        <td class="align-middle"><?= floatval($bag['weight_volume']) ?> m³</td>
                                        ...
                                        <td class="align-middle">
                                            <div class="d-flex gap-1">
                                                <a href="<?= base_url('admin/bags-packing?id=' . $bag['id']) ?>" class="btn btn-sm btn-soft-warning" onclick="event.stopPropagation();"><i class="ri-pencil-line me-1"></i><?= __('Sửa') ?></a>
                                                <?php if ($bag['status'] === 'open'): ?>
                                                <button type="button" class="btn btn-sm btn-soft-success btn-seal-bag" data-id="<?= $bag['id'] ?>" data-code="<?= htmlspecialchars($bag['bag_code']) ?>" data-count="<?= $bag['total_packages'] ?>"><i class="ri-lock-line me-1"></i><?= __('Đóng bao') ?></button>
                                                <button type="button" class="btn btn-sm btn-soft-danger btn-delete-bag" data-id="<?= $bag['id'] ?>" data-code="<?= htmlspecialchars($bag['bag_code']) ?>"><i class="ri-delete-bin-line me-1"></i><?= __('Xóa') ?></button>
                                                <?php elseif ($bag['status'] === 'sealed'): ?>
                                                <button type="button" class="btn btn-sm btn-soft-info btn-unseal-bag" data-id="<?= $bag['id'] ?>" data-code="<?= htmlspecialchars($bag['bag_code']) ?>"><i class="ri-lock-unlock-line me-1"></i><?= __('Mở bao') ?></button>
                                                <button type="button" class="btn btn-sm btn-soft-danger btn-delete-bag" data-id="<?= $bag['id'] ?>" data-code="<?= htmlspecialchars($bag['bag_code']) ?>"><i class="ri-delete-bin-line me-1"></i><?= __('Xóa') ?></button>
                                                <?php endif; ?>
                                            </div>
                                        </td>
...
$(function(){
    ...
    $('#btn-export-bags').on('click', function(){
        ...
        window.location.href = '<?= base_url('ajaxs/admin/bags-export.php') ?>?' + params.toString();
    });
});
</script>
