...
?>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form method="GET" action="<?= base_url('admin/shipments-list') ?>">
                            ...
                            <div class="row g-3 align-items-end">
                                ...
                                <div class="col-md-auto">
                                    ...
                                    <a href="<?= base_url('admin/shipments-list') ?>" class="btn btn-secondary"><?= __('Reset') ?></a>
                                </div>
                            </div>
                        </form>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr><td colspan="10" class="text-center text-muted py-4"><?= __('Chưa có chuyến xe nào') ?></td></tr>
                                    <?php endif; ?>
                                    <?php $rowNum = $offset; foreach ($shipments as $s): $rowNum++; ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex gap-1">
                                                <a href="<?= base_url('admin/shipments-detail?id=' . $s['id']) ?>" class="btn btn-sm btn-soft-info"><i class="ri-eye-line me-1"></i><?= __('Xem') ?></a>
                                                <?php if ($s['status'] === 'preparing'): ?>
                                                <button type="button" class="btn btn-sm btn-soft-danger btn-delete-shipment" data-id="<?= $s['id'] ?>" data-code="<?= htmlspecialchars($s['shipment_code']) ?>"><i class="ri-delete-bin-line me-1"></i><?= __('Xóa') ?></button>
                                                <?php endif; ?>
                                            </div>
                                        </td>
                                    </tr>
...
$(function(){
        ...
        $.post(ajaxUrl, {
            ...
            if (res.status === 'success') {
                Swal.fire({icon: 'success', title: res.msg, timer: 1500, showConfirmButton: false}).then(function(){
                    window.location.href = '<?= base_url('admin/shipments-detail?id=') ?>' + res.shipment_id;
                });
            } else {
    ...
    $('#btn-export-excel').on('click', function(){
        ...
        window.location.href = '<?= base_url('ajaxs/admin/shipments-list-export.php') ?>?' + params.toString();
    });
