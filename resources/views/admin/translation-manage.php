            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center">
                        ...
                        <button type="button" class="btn btn-outline-danger me-2" id="btn-cleanup-orphans">
                            <i class="ri-delete-bin-line me-1"></i><?= __('Dọn bản dịch thừa') ?>
                        </button>
...
$(function(){
    ...
    $('#btn-cleanup-orphans').on('click', function(){
        var $btn = $(this);
        Swal.fire({
            title: '<?= __("Dọn bản dịch thừa") ?>?',
            html: '<?= __("Xóa tất cả bản dịch có key không còn được dùng trong code") ?>?',
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $btn.prop('disabled', true).html('<i class="ri-loader-4-line ri-spin me-1"></i><?= __("Đang dọn...") ?>');
                $.post(ajaxUrl, {
                    request_name: 'cleanup_orphans',
                    csrf_token: csrfToken
                }, function(res){
                    if (res.status === 'success') {
                    ...
                    $btn.prop('disabled', false).html('<i class="ri-delete-bin-line me-1"></i><?= __("Dọn bản dịch thừa") ?>');
                }, 'json').fail(function(){
                    Swal.fire({icon: 'error', title: 'Error', text: '<?= __("Lỗi kết nối") ?>'});
                    $btn.prop('disabled', false).html('<i class="ri-delete-bin-line me-1"></i><?= __("Dọn bản dịch thừa") ?>');
                });
