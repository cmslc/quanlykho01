...
?>
        ...
        <!-- Filter Bar -->
        <?php
        $baseUrl = base_url('staffcn/shipping-calculator');
                                ...
                                <tbody>
                                    ...
                                    <tr class="<?= $warnClass ?>">
                                        ...
                                        <td class="align-middle">
                                            <a href="<?= base_url('staffcn/bags-packing?id=' . $bag['bag_id']) ?>"><strong><?= htmlspecialchars($bag['bag_code']) ?></strong></a>
...
$(function(){
    ...
    var calcAjaxUrl = '<?= base_url('ajaxs/staffcn/shipping-calculator.php') ?>';
    var expandedOrders = {};
    ...
    $(document).on('click', '.btn-expand-pkgs', function(e){
        ...
        $.post(pkgAjaxUrl, { request_name: 'get_order_packages', order_id: orderId, csrf_token: csrfToken }, function(res){
            if (res.status === 'success') {
                var statusLabels = { 'cn_warehouse': '<?= __('Đã về kho Trung Quốc') ?>', 'vn_warehouse': '<?= __('Đã về kho Việt Nam') ?>', 'packed': '<?= __('Đã đóng bao') ?>', 'loading': '<?= __('Đang xếp xe') ?>', 'shipping': '<?= __('Đang vận chuyển') ?>', 'delivered': '<?= __('Đã giao') ?>' };
