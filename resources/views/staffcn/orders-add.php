...
$preselect_type = input_get('product_type') ?: 'retail';
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="status">
                                        <option value="cn_warehouse" selected><?= __('Kho TQ') ?></option>
...
$('#form-quick-customer').on('submit', function(e){
    ...
    $.ajax({
        url: '<?= base_url('ajaxs/staffcn/customers.php') ?>',
        type: 'POST',
        data: $(this).serialize(),
        dataType: 'json',
        success: function(res){
        ...
        success: function(res){
            if(res.status == 'success'){
                Swal.fire({icon: 'success', title: res.msg, showConfirmButton: true, confirmButtonText: 'OK'}).then(function(){
                    window.location.href = '<?= base_url('staffcn/orders-detail') ?>?id=' + res.order_id;
                });
            } else {
