...
?>
        ...
        <form id="form-edit-order">
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="status">
                                    <option value="cn_warehouse" <?= $order['status'] === 'cn_warehouse' ? 'selected' : '' ?>><?= __('Kho TQ') ?></option>
                                    <option value="packed" <?= $order['status'] === 'packed' ? 'selected' : '' ?>><?= __('Đã đóng bao') ?></option>
                                    <option value="shipping" <?= $order['status'] === 'shipping' ? 'selected' : '' ?>><?= __('Vận chuyển') ?></option>
                                    <option value="vn_warehouse" <?= $order['status'] === 'vn_warehouse' ? 'selected' : '' ?>><?= __('Kho VN') ?></option>
                                    <option value="delivered" <?= $order['status'] === 'delivered' ? 'selected' : '' ?>><?= __('Đã giao') ?></option>
            ...
            <div class="d-flex gap-2 mb-4">
                ...
                <a href="<?= base_url('staffcn/orders-detail?id=' . $order['id']) ?>" class="btn btn-secondary"><?= __('Quay lại') ?></a>
            </div>
        </form>
...
$(document).on('click', '.btn-delete-group', function(){
    ...
    Swal.fire({
        ...
        function next(i){
            ...
            $.post('<?= base_url('ajaxs/staffcn/packages.php') ?>', {request_name:'delete', id: ids[i], csrf_token:'<?= $csrf->get_token_value() ?>'}, function(){ deleted++; Swal.update({text: deleted+'/'+count}); next(i+1); }, 'json').fail(function(){ next(i+1); });
        }
        next(0);
    });
});
...
$('#form-edit-order').on('submit', function(e){
    ...
    $.ajax({
        url: '<?= base_url('ajaxs/staffcn/orders.php') ?>',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function(res){
            if(res.status == 'success'){
                Swal.fire({icon: 'success', title: res.msg, timer: 1500, showConfirmButton: false}).then(function(){
                    window.location.href = '<?= base_url('staffcn/orders-detail?id=' . $order['id']) ?>';
                });
            } else {
...
$(document).on('click', '.btn-delete-pkg', function(){
    ...
    Swal.fire({
        ...
        if(result.isConfirmed){
            $.post('<?= base_url('ajaxs/staffcn/packages.php') ?>', {
                request_name: 'delete',
                id: pkgId,
...
$('#btn-create-package').on('click', function(){
    ...
    function createOne(remaining){
        ...
        $.post('<?= base_url('ajaxs/staffcn/packages.php') ?>', postData, function(res){
