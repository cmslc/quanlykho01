...
$existingCats = $ToryHub->get_list_safe("SELECT DISTINCT category FROM `expenses` WHERE `warehouse` = 'cn' ORDER BY category ASC", []);
...
$_mRow = $ToryHub->get_row_safe("SELECT COALESCE(SUM(amount),0) as total, COALESCE(SUM(amount / COALESCE(NULLIF(exchange_rate,0), ?)),0) as total_cny FROM `expenses` WHERE `expense_date` BETWEEN ? AND ? AND `warehouse` = 'cn'", [$exchangeRate, $monthStart, $monthEnd]);
$totalMonth = floatval($_mRow['total']);
$totalMonthCny = floatval($_mRow['total_cny']);
$countMonth = $ToryHub->num_rows_safe("SELECT id FROM `expenses` WHERE `expense_date` BETWEEN ? AND ? AND `warehouse` = 'cn'", [$monthStart, $monthEnd]) ?: 0;
...
$_pRow = $ToryHub->get_row_safe("SELECT COALESCE(SUM(amount),0) as total, COALESCE(SUM(amount / COALESCE(NULLIF(exchange_rate,0), ?)),0) as total_cny FROM `expenses` WHERE `expense_date` BETWEEN ? AND ? AND `warehouse` = 'cn'", [$exchangeRate, $prevMonthStart, $prevMonthEnd]);
$totalPrevMonth = floatval($_pRow['total']);
$totalPrevMonthCny = floatval($_pRow['total_cny']);
$changePercent = ($totalPrevMonth > 0) ? round(($totalMonth - $totalPrevMonth) / $totalPrevMonth * 100, 1) : 0;

// Tổng theo danh mục tháng đã chọn
$catSums = $ToryHub->get_list_safe("SELECT category, COALESCE(SUM(amount),0) as total, COUNT(*) as cnt FROM `expenses` WHERE `expense_date` BETWEEN ? AND ? AND `warehouse` = 'cn' GROUP BY category ORDER BY total DESC", [$monthStart, $monthEnd]);

// Năm có dữ liệu
$yearsData = $ToryHub->get_list_safe("SELECT DISTINCT YEAR(expense_date) as y FROM `expenses` WHERE `warehouse` = 'cn' ORDER BY y DESC", []);
            ...
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    ...
                    <div class="d-flex gap-2">
                        <a href="<?= base_url('ajaxs/staffcn/expenses-export.php') ?>?month=<?= $filterMonth ?>&year=<?= $filterYear ?>&category=<?= urlencode($filterCat) ?>" class="btn btn-success">
                            <i class="ri-file-excel-2-line"></i> <?= __('Xuất Excel') ?>
                        </a>
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalAddExpense">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-end justify-content-between mt-2">
                            <div>
                                ...
                                <small class="text-muted"><?= format_cny($totalMonthCny) ?></small>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-end justify-content-between mt-2">
                            <div>
                                ...
                                <small class="text-muted"><?= format_cny($totalPrevMonthCny) ?></small>
                            </div>
                            <div class="avatar-sm flex-shrink-0">
                    ...
                    <div class="card-body">
                        <form method="GET" action="<?= base_url('staffcn/expenses') ?>">
                            <div class="row g-3 align-items-end">
                                ...
                                <div class="col-md-3">
                                    <button type="submit" class="btn btn-primary"><i class="ri-search-line"></i> <?= __('Lọc') ?></button>
                                    <a href="<?= base_url('staffcn/expenses') ?>" class="btn btn-outline-secondary"><?= __('Reset') ?></a>
                                </div>
                            </div>
                        </form>
                    </div>
                            ...
                            <table class="table data-table table-hover mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <?php $_expRate = floatval($exp['exchange_rate'] ?? 0) > 0 ? floatval($exp['exchange_rate']) : $exchangeRate; ?>
                                        <td><?= format_dual($exp['amount'], $exp['amount'] / $_expRate, true, true) ?></td>
                                        <td class="text-end"><small class="text-muted"><?= $exp['exchange_rate'] ? fnum($exp['exchange_rate'], 0) : '-' ?></small></td>
                                        <td><?= htmlspecialchars($exp['description'] ?? '') ?></td>
                                        <td><?= $exp['expense_date'] ?></td>
                                        <td><?= htmlspecialchars($exp['created_by_name'] ?? '') ?></td>
                                        <td>
                                            <button class="btn btn-sm btn-soft-info btn-edit-expense" data-id="<?= $exp['id'] ?>" data-category="<?= htmlspecialchars($exp['category']) ?>" data-amount="<?= $exp['amount'] ?>" data-exchange-rate="<?= $exp['exchange_rate'] ?? 0 ?>" data-date="<?= $exp['expense_date'] ?>" data-description="<?= htmlspecialchars($exp['description'] ?? '') ?>">
                    ...
                    <form id="form-add-expense">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control amount-cny-input" name="amount" min="0" step="1" required>
                                <div class="form-text amount-vnd-preview"></div>
                            </div>
                            <div class="mb-3">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form id="form-edit-expense">
                        ...
                        <input type="hidden" name="stored_exchange_rate" id="edit-stored-rate">
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label"><?= __('Danh mục') ?> <span class="text-danger">*</span></label>
                            ...
                            <div class="mb-3">
                                <label class="form-label"><?= __('Số tiền') ?> (¥) <span class="text-danger">*</span></label>
                                <input type="number" class="form-control amount-cny-input" name="amount" id="edit-amount" min="0" step="1" required>
                                <div class="form-text amount-vnd-preview"></div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label"><?= __('Ngày chi') ?> <span class="text-danger">*</span></label>
...
$(function() {
    ...
    function fmtVnd(v) { return new Intl.NumberFormat('vi-VN').format(Math.round(v)) + 'đ'; }

    // Live CNY → VND preview (edit form uses stored rate)
    $(document).on('input', '.amount-cny-input', function(){
        var cny = parseFloat($(this).val()) || 0;
        var $preview = $(this).siblings('.amount-vnd-preview');
        var rate = $(this).closest('#form-edit-expense').length ? (parseFloat($('#edit-stored-rate').val()) || exchangeRate) : exchangeRate;
        $preview.html(cny > 0 ? '≈ <strong class="text-danger">' + fmtVnd(cny * rate) + '</strong>' : '');
    });
    ...
    // Edit expense - open modal (amount stored as VND, convert to CNY using stored rate)
    $(document).on('click', '.btn-edit-expense', function() {
        var amountVnd = parseFloat($(this).data('amount')) || 0;
        var storedRate = parseFloat($(this).data('exchange-rate')) || 0;
        var useRate = storedRate > 0 ? storedRate : exchangeRate;
        var amountCny = useRate > 0 ? Math.round(amountVnd / useRate) : 0;
        $('#edit-id').val($(this).data('id'));
        $('#edit-stored-rate').val(useRate);
        $('#edit-category').val($(this).data('category'));
        $('#edit-amount').val(amountCny).trigger('input');
