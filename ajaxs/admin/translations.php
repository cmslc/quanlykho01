...
if ($request === 'cleanup_orphans') {
    ...
    // Get all translation names in DB
    $allTranslations = $ToryHub->get_list_safe("SELECT DISTINCT `name` FROM `translate`", []);
    $orphanNames = [];
    ...
    if (empty($orphanNames)) {
        echo json_encode(['status' => 'success', 'deleted' => 0, 'msg' => 'Không có bản dịch thừa']);
        exit;
    }
    ...
    foreach ($orphanNames as $name) {
        $ToryHub->remove_safe('translate', '`name` = ?', [$name]);
        $deleted++;
    }
