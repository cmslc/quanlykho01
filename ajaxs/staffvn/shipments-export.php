<?php
/**
 * Export shipment detail to XLSX with product images (StaffVN)
 */
define('IN_SITE', true);
...
// Header row
$headers = [__('Ngày tạo'), __('Mã hàng'), __('Khách hàng'), __('Số kiện'), __('Tổng cân (kg)'), __('Tổng khối (m³)')];
foreach ($headers as $ci => $h) {
    $sheet->setCellValue(Coordinate::stringFromColumnIndex($ci + 1) . '1', $h);
}
if ($maxImgs === 0) {
    $sheet->setCellValue(Coordinate::stringFromColumnIndex(7) . '1', __('Ảnh'));
} else {
    for ($i = 1; $i <= $maxImgs; $i++) {
        $label = $maxImgs === 1 ? __('Ảnh') : __('Ảnh') . ' ' . $i;
...
foreach ($grouped as $maHang => $group) {
    ...
    $sheet->setCellValue('C' . $rowNum, $group['customer']);
