...
if ($request === 'add') {
    ...
    $insertResult = $ToryHub->insert_safe("orders", [
        ...
        'cn_tracking' => strtoupper(trim(input_post('tracking_number'))),
        'intl_tracking' => '',
        'vn_tracking' => '',
...
if ($request === 'edit') {
    ...
    $edit_weight_charged = calculate_charged_weight($edit_weight_actual, floatval($order['weight_volume'] ?? 0));
    ...
    if (!empty($edit_packages)) {
        ...
        foreach ($edit_packages as $pkgId => $pkgData) {
            ...
            $pkgUpdate = [
                'length_cm' => $pkg_length, 'width_cm' => $pkg_width,
                'height_cm' => $pkg_height, 'weight_volume' => round($pkg_vol, 2),
            ];
