...
class Packages extends DB
{
    ...
    public function syncOrderStatus($order_id, $changed_by)
    {
        ...
        if (empty($activePackages)) {
            ...
            // Wholesale gửi từng phần: bỏ qua kiện cn_warehouse (chưa gửi) khi tính rank
            if ($order['product_type'] === 'wholesale') {
                $shippedPkgs = array_filter($activePackages, function($p) {
                    return $p['status'] !== 'cn_warehouse';
                });
                if (!empty($shippedPkgs)) {
                    $activePackages = $shippedPkgs;
                }
            }
            $pkgRank = ['pending' => 0, 'cn_warehouse' => 1, 'packed' => 2, 'loading' => 3, 'shipping' => 4, 'vn_warehouse' => 5, 'delivered' => 6];
            $minRank = 999;
            foreach ($activePackages as $pkg) {
                $rank = $pkgRank[$pkg['status']] ?? -1;
                if ($rank >= 0 && $rank < $minRank) $minRank = $rank;
            }
            $rankToStatus = [0 => 'pending', 1 => 'cn_warehouse', 2 => 'packed', 3 => 'loading', 4 => 'shipping', 5 => 'vn_warehouse', 6 => 'delivered'];
        ...
        if ($derivedStatus !== $order['status']) {
            $dateFields = [
                'cn_warehouse' => 'cn_warehouse_date', 'packed' => 'packed_date',
                'loading' => 'loading_date', 'shipping' => 'shipping_date',
